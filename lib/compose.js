const UnaryCall = require("./calls/unaryCall");
const ClientStreamCall = require("./calls/clientStreamCall");
const ServerStreamCall = require("./calls/serverStreamCall");
const DuplexCall = require("./calls/duplexCall");
...
module.exports = function compose(
  ...
  // turning middlewareStack into an array to iterate through later
  ...
  switch (methodType) {
    case "Duplex":
      ...
      return async function(call) {
        console.log("duplex call", call);
        console.log("duplex call proto", call.__proto__);
        let duplexCall = new DuplexCall(call);
    ...
    case "ClientStream":
      ...
      return async function(call, callback) {
        ...
        let clientStreamCall = new ClientStreamCall(call, callback);
        for (let i = 0; i < middlewareStack.length; i++) {
    ...
    case "ServerStream":
      // class Context {}
      return async function(call) {
        let serverStreamCall = new ServerStreamCall(call);
        for (let i = 0; i < middlewareStack.length; i++) {
    ...
    case "Unary":
      return async function(call, callback) {
        let unaryCall = new UnaryCall(call, callback);
        for (let i = 0; i < middlewareStack.length; i++) {
