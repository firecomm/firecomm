...
module.exports = function build(PROTO_PATH, config = {
  ...
  switch (true) {
    case config.keepCase !== true && config.keepCase !== false:
      throw new Error('BUILD ERROR: config object\'s keepcase must be a boolean'); 
    case config.longs !== Number && config.longs !== String:
      throw new Error('BUILD ERROR: config object\'s longs must be a Number or a String'); 
    ...
    case typeof config.defaults !== false && config.defaults !== true:
      throw new Error('BUILD ERROR: config object\'s defaults must be a boolean'); 
