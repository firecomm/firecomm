...
server.addService( 
   ...
   *
   * @param {string|[string]} PORT Socket in the form of `IP`:`PORT` for Firecomm `Server` class to listen on.
   * @param {{}=} [config] if no config is supplied, an insecure channel will be created (no TLS handshake required and traffic unencrypted, to create a secure connection, supply the paths to an RSA private key and certificate
   * @example const { Server } = require( "firecomm" );
