...
// compose basically takes in an argument ( {handler: arrayOfKeys, middlewareStack: handlerObject[arrayOfKeys[i]]}, service )
  // -> finds which method is being currently used -> runs middleware-like functionality
...
module.exports = class Server {
  ...
  addService(serviceDefinition, handlerObject, serviceMiddleware) {
    ...
    // handlerObject ~= { unaryChat: [waitFor, unaryChat], serverStream, clientStream, bidiChat }
 ...
 */
  ...
  //          else if there are multiple ports, iterate and open mutiple channels
  // PORT === 0.0.0.0:3000 
  //  // PORT is not the best name. Its actually a *socket*. Socket = Port + IP Address
  bind(PORT, config) {
    ...
    for (let i = 0; i < ports.length; i++) {
      ...
      // else, take the contents of config (certificate and private key) and apply them to create an encrypted connection
      if (!config) {
        ...
        // create an array which is "A list of private key and certificate chain pairs to be used for authenticating the server"        
        let listOfObjects =
